...
public class BoardManager {
   ...
   public String boardUpdate(BoardVO vo)
   {
	   ...
	   for(int i=0;i<bList.size();i++)
	   {
		   // remove(index) . set(index) <- 수정 삭제 시 인덱스 번호가 필요하다
		   BoardVO pVO=bList.get(i);
				   ...
				   if(pVO.getNo()==vo.getNo())
				   {
					   if(pVO.getPwd().equals(vo.getPwd()))
					   {
						   // 수정 (비밀번호가 일치)
						   result="YES";  
						 //  bList.set(i, vo);        //  메모리에서 변경 => 수정
						   // set = 새로운 객체가 들어감 (날짜에서 문제가 생김) new 데이터 다 집어넣어야 하니까 기존에 있는 데이터만 바꿔치기한다.
						   pVO.setContent(vo.getContent());
						   pVO.setName(vo.getName());
						   pVO.setSubject(vo.getSubject());
						  
						   fileSave();                //  파일에서 변경 => 수정된 내용을 파일에 저장
						  // 메모리 저장 == 파일에 저장
					   }
					   else
					   {
